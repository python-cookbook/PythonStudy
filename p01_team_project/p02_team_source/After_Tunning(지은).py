...
class Main(Game):
    ...
    def state_color(column, CNT1, CNT2, color1, color2="white"):        #지은 #tunning
        col1 = [color1 for i in range(CNT2)]        
        col2 = [color2 for i in range(CNT1-CNT2)]   # CNT1: 색깔 표시 개수 ["white","white","white"]
        column = col1 + col2                        # CNT2: 색깔이 표시되는 경우의 수  self.game.BALL_CNT : 0~3            
        return column
        
        state_color(self.ball_color, 3, self.game.BALL_CNT, "orange")
        state_color(self.game.STRIKE_CNT, 2, self.strike_color, "blue")
        state_color(self.game.OUT_CNT, 2, self.out_color, "red")
