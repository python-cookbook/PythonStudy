...
### 실시간 선수 기록 저장 ####
save_player_path = "c:\\data\\basebal2_save_player12.csv"


### 최종 경기 기록 저장 ###
save_result_path = "c:\\data\\basebal2_save_result12.csv"
...
load_player_path = "c:\\data\\baseball_save_player10.csv"
        ...
        try:
            if load_path == None :
                ...
                for record in records:
                    curr_team = Game.hometeam if Game.CHANGE == 0 else Game.awayteam
                    player = Game.select_player(Game.BATTER_NUMBER[Game.CHANGE] , curr_team.player_list)
                    if record[1] == player.name :
                        player.number, player.name, player.record.avg, player.record.bob, player.record.homerun = record
                        #Main.INNING, Main.curr_team.team_name, Player.number, Player.name, Player.record.avg, Player.record.bob, Player.record.homerun = record              
...
class Main(Game):
    ...
    def attack(self):
        ...
        if Game.OUT_CNT < 3:
            ...
            while True:
                ...
                #save_player=[Game.INNING, curr_team.team_name, player.number, player.name, player.record.avg, player.record.bob, player.record.homerun]       #지은
    ...
    def start_game(self):
        ...
        Saveandload.load_record(load_player_path)   #지은
