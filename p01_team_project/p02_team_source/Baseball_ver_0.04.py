...
class Saveandload:
    ...
    def make_data_set(self,player_info,cnt,game_info,adv):
        ...
        self.adv = adv
    # 저장할때 join 을 통해 저장하다보니, str타입이 필요했음
    # 어차피 리스트 형태로 매개변수를 받다보니, 아래 코드를 통해, 리스트 요소에 접근하여 str으로 변환함.
    # 사실 save함수에서바꿔도됨.
    ...
    # writerow로 저장하려 했으나, 잘 구동되지 않아 아래와 같이 함.
    # 파일명은 홈팀VS어웨이팀.txt로 저장됨
    # csv파일은 with open으로 csv저장할 때, 한글파일이 있는경우 인코딩 문제가 해결되지 않아 쨟꺍 거림..
    # 위 문제는 용현이형한테도 여쭤보았으나, 본인도 이 문제를 해결하지 못했다고 함!
        with open(Saveandload.FILE_PATH + self.__hometeam.team_name+"VS"+self.__awayteam.team_name+".txt", "wt", encoding="utf-8") as f:
            for line in Saveandload.DATA_SET:
                for idx, data_list in enumerate(line, 1):
                    if idx != 4:
...
class Game:
    ...
    def attack(self):
        ...
        if Game.OUT_CNT < 3:
            ...
            while True:
                # 저장되는 변수를 아래 리스트에 담아 make_data_set함수의 매개변수로 넣습니다.
                PLAYER_INFO = [player.number, player.name, player.record.avg, player.record.bob,player.record.homerun]
                CNT = [Game.STRIKE_CNT, Game.BALL_CNT, Game.OUT_CNT]
                GAME_INFO = [Game.INNING, Game.CHANGE]
                ADV = Game.ADVANCE
                Saveandload().make_data_set(PLAYER_INFO, CNT, GAME_INFO, ADV)
                ...
                try :
                    hit_yn = int(input('타격을 하시겠습니까?(타격 : 1 타격안함 : 0  저장 : 333)'))
                except Exception:
                ...
                if hit_yn == 1:#################타격 시############################ #융
                    ...
                    if hit_cnt[0] == 0:  # strike !!!
                        ...
                        if hit_cnt[1] == True:#파울일 때
                            if Game.STRIKE_CNT <= 1: #스트라이크 카운트가 1 이하일때는 원래대로 진행 융
                                Game.STRIKE_CNT += 1
                                print('== ▣ 파울!!!\n')
                                print('== ▣ 스트라이크!!!\n')   #여기도 나중에 func(0,0,1) 이런식으로 하면 됨.  원주
                                if Game.STRIKE_CNT == 3:
                                    print('== ▣ 삼진 아웃!!!\n')
                                    Game.STRIKE_CNT = 0
                                    Game.OUT_CNT += 1
                        ...
                        Game.STRIKE_CNT = 0     #진루했으니, 0처리
                        if hit_cnt[0] != 4:     #1,2,3루타 라면
                            print('== ▣ {}루타!!!\n'.format(hit_cnt[0]))
                            player.hit_and_run(1, 0, 0) # 1,0,0 #원주
                        else:
                            print('== ▣ 홈런!!!\n')
                            player.hit_and_run(1, 0, 1)  # 1,0,1 #원주
                    ...
                    if (random_numbers[1]>=6 and random_numbers[1]<=8) or (random_numbers[1]>=11 and random_numbers[1]<=13) or (random_numbers[1]>=16 and random_numbers[1]<=18):
                        ...
                        if Game.STRIKE_CNT ==3:
                            ...
                            player.hit_and_run(0, 0, 0)
                            break
            #일단 333입력하면 저장하도록 했습니다.
            #무성이형이 액션기능 넣어서 활성화해주세욬ㅋㅋ
                elif hit_yn == 333:
                    Saveandload().save()
            PLAYER_INFO = [player.number, player.name, player.record.avg, player.record.bob, player.record.homerun]
            CNT = [Game.STRIKE_CNT, Game.BALL_CNT, Game.OUT_CNT]
