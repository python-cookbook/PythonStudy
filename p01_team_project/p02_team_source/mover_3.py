...
class Saveandload:
    ...
    @staticmethod
    def make_data_set(cnt, game_info, adv):
        ...
        DATA_SET = []
        cnt = [str(data) for data in cnt]
        game_info = [str(data) for data in game_info]
        adv = [str(data) for data in adv]
        DATA_SET.append([game_info, adv, cnt])
    ...
    def save(DATA_SET):
        ...
        with open(Saveandload.FILE_PATH + "test.txt", "wt", encoding="utf-8") as f:
            print('여기',DATA_SET)
            for row in DATA_SET:
                for idx,value in enumerate(row,1):
                    ...
                    if idx == 3:
                        f.write(value[0]+","+value[1]+","+value[3]+'\n')
    ...
    def load():
        ...
        BATTER_NUMBER = 0
        import csv
        f = open('d:/data/test.txt')      #파일명이 바뀌어야 할 것.
        ...
        return [INNING, adv, CHANGE, STRIKE_CNT, BALL_CNT, OUT_CNT, SCORE, BATTER_NUMBER]
...
class Game(object):
    ...
    # INNING = 0
    INNING = 1 if Saveandload.LOAD_YN == False else temp[0]
    print(INNING)

    # CHANGE = 0  # 0 : hometeam, 1 : awayteam
    CHANGE = 0 if Saveandload.LOAD_YN == False else temp[2]
    print(CHANGE)

    # ADVANCE = [0, 0, 0]  # 진루 상황
    ADVANCE = [0,0,0] if Saveandload.LOAD_YN == False else temp[1]
    print(ADVANCE)


    STRIKE_CNT = 0 if Saveandload.LOAD_YN == False else temp[3]
    BALL_CNT = 0 if Saveandload.LOAD_YN == False else temp[4]
    OUT_CNT = 0  if Saveandload.LOAD_YN == False else temp[5]
    print(STRIKE_CNT,BALL_CNT,OUT_CNT)

    # SCORE = [0, 0]  # [home, away]
    SCORE = [0, 0]  if Saveandload.LOAD_YN == False else temp[6]
    print(SCORE)
    # BATTER_NUMBER = [1, 1]  # [home, away] 타자 순번
    BATTER_NUMBER = [1, 1] if Saveandload.LOAD_YN == False else temp[7]
    print(BATTER_NUMBER)
