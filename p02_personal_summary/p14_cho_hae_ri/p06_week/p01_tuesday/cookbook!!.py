...
class IStream(metaclass=ABCMeta):
    ...
    def write(self, data):
        pass
a = IStream()
# 위의 코드는 에러가 난다 . 추상베이스 클래스의 주요 기능은 직접 인스턴스화 할수 없다는 점이다.
from abc import ABCMeta, abstractmethod
...
class B:
    ...
    def __getattr__(self, name):
        return getattr(self._a, name)
# 또한 __getattr__() 메소드는 밑줄 두개로 시작하는 대부분의 특별 메소드에 적용되지 않는다는 점도 중요하다.
class ListLike:
    def __init__(self):
        self._items = []
def __getattr__(self, name):
...
클래스의 __new__() 메소드를 호출해서 초기화 하지 않는 인스턴스를 생성할 수 있다.
class Date:
    def __init__(self, year, month, day):
        self.year = year
        self.month = month
...
문자열로 저장된 메소드 이름을 가지고 있고, 이 메소드를 실행하고 싶다면 getattr()를 사용하면 된다.
import math
class Point:
