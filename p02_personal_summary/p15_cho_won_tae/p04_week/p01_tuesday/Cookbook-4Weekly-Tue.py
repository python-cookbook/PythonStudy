...
# 해결
# 단순한 XML 문서에서 데이터를 얻기 위해 xml.etree.ElementTree 모듈을 사용하면 된다.
# Planet Python에서 RSS 피드를 받아 파싱을 해야 한다고 가정해보자
from urllib.request import urlopen
...
for item in doc.iterfind('channel/item'):
    ...
    print(link)
    print()
# 많은 어플리케이션에서 XML 로 인코딩된 데이터를 다룬다.
# 인터넷 상에서 데이터를 주고 받을 때 XML을 사용하는 곳이 많기도 하지만, 애플리케이션 데이터를 저장할 때도 일반적으로 사용하는 형식이다. 이 뒤로 나오는 토론은 독자가 XML 기본 내용에 이미 익숙하다고 가정한다
# 많은 경우, XML 이 데이터를 저장할 때 사용되면 문서구조는 단순하고 이해하기 쉽다.
# 예를 들어 앞에 나온 예제의 RSS 피드는 아마도 다음과 비슷할 것이다
<?xml version="1.0"?>
...
# xml.etree.ElementTree.parse() 함수가 XML문서를 파싱하고 문서 객체로 만든다. 여기서부터 특정XML 요소를 찾기위해
# find(), iterfind(), findtext() 와 같은 함수를 사용한다. 함수에 사용하는 인자는 channel/item 또는
# title과 같이 특정 태그의 이름을 사용한다
# 태그를 명시할때, 전체 문서 구조체를 고려해야 한다.
# 모든 찾기 작업을 시작요소에 상대적으로 발생한다. 마찬가지로 함수에 전달하는 태그 이름 역시 시작에 상대적이다
# ElementTree 모듈이 나타내는 모든 요소는 파싱에 유용한 요소와 메소드를 약간 가지고 있다.
# tage 요소에는 태그의 이름, text 요소에는 파싱에 유용한 요소와 메소드를 약간 가지고 있다.
...
# 해결
# 중분 데이텇 ㅓ리에 직면할 떄면 언제나 이터레이터와 제너레이터를 떠올려야 한다
    ...
    for event, elem in doc:
        if event == 'start':
            ...
            try:
                ...
                pass
# 함수를 테스트하기 위해서 예로 사용할 커다란 XML 파일이 필요하다
# 이런 파일은 정부 사이트나 데이터 웹사이트에서 찾을 수 있다
<response>
    <row>
        <row ...>
...
# 도로의 움푹 패인 곳의 숫자를 사용하여 ZIP 코드별로 순위를 매기는 스크립트를 작성한다고 가정하자
from xml.etree.ElementTree import parse
...
for zipcode, num in potholes_by_zip.most_common():
    print(zipcode, num)
# 이 스크립트는 전체를 읽어 메몰에 넣는다는 문제점이 있기에 아래의 레시피 코드를 사용해보자
from collections import Counter
potholes_by_zip = Counter()
