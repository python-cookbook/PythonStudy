...
2장 8절 여러 줄에 걸친 정규 표현식 사용 : re.compile() 함수에 re.DOTALL 이라는 플래그로 .을 사용해서 텍스트 매핑을 할 수 있다.


2장 9절 유니코드 텍스트 노멀화 : 유니코드 문자열 작업을 하고 있을 때 모든 문자열에 동일한 표현식을 갖도록 하기 위해 unicodedata 모듈로 텍스트를 노멀라이제이션 해야한다.
- normalize()의 첫 번째 인자엔 문자열을 어떻게 노멀화 할 것인지 지정해준다.
- combining() 함수는 문자가 결합 문자인지 확인한다.

2장 10절 정규 표현식에 유니코드 사용 : 텍스트 프로세싱에 정규표현을 사용 할 때 유니코드 문자 처리는 유니코드 문자에 이스케이프 시퀀스를 사용한다.


2장 11절 문자열에서 문자 잘라내기 : 텍스트의 처음, 끝, 중간에서 원하지 않는 공백을 잘라내기 위해 strip() 메소드를 사용하되 문자열 중간의 공백을 제거하려면 replace()나
                               정규표현식의 치환을 이용해야 한다.

2장 12절 텍스트 정리 : 

2장 13절 텍스트 정렬 : 텍스트를 특정형식에 맞추어 정렬하는 경우 정렬 메소드로 ljust(), rjust(), center() 등이 존재한다. 정렬 함수에 format() 함수를 사용 할 수도 있다.

2장 14절 문자열 합치기 : 작은 문자열을 긴 문자열 하나로 합치는 경우 합치고자 하는 문자열이 시퀀스나 순환 객체 안에 있다면 join() 메소드를 사용한다.

2장 15절 문자열에 변수 사용 : 문자열에 변수를 넣고 변수에 맞는 값을 채우고 싶은 경우 format() 메소드를 응용해서 비슷한 기능을 구현해야 한다. 파이썬 자체적으로 지원하지는 않는다.

2장 16절 텍스트 열의 개수 고정 : 긴 문자열의 서식을 바꿔 열의 개수를 조절 할 경우 textwrap 모듈을 사용해서 텍스트를 리포맷 하면 된다.
