...
## 요 코드는 Person을 상속받아 name property에 새로운 기능을 넣어 클래스를 확장한다
...
# 읽기 전용 속성을 프로퍼티로 정의하고, 이 속성에 접근할 때만 계산하고 싶다. 하지만 한 번 접근하고 나면 이 값을
# 캐시해놓고 다음 번에 접근할 때에는 다시 계산하지 않았으면 싶다
################################################################################################################
...
# 제너럴한 목적으로(...) __init__() 메소드를 정의하는 기술은 규모가 작은 자료 구조를 대량으로 만들때 유용하다
